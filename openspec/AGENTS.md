...
- Distinguish Cherri value types (`text`, `number`, `bool`, `dictionary`, `array`) from Shortcuts content-item coercions.
- Match action and function parameter types explicitly; when uncertain, check with `typeOf(...)` before passing values onward.
- Prefer typed function signatures and typed return values for predictable contracts across includes.
- Use explicit coercion patterns (`text(...)`, `number(...)`, `getDictionary(...)`) at boundaries where provider or model responses are dynamic.
- Keep output-surface expectations clear: use actions like `show(...)`, `quicklook(...)`, or output actions only with compatible types.
...
- Types and content item types: https://www.cherrilang.org/language/types/
